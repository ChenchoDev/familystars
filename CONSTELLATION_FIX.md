# FamilyStars — Constellation.jsx completo y corregido

## Problema

El archivo Constellation.jsx quedó corrupto al aplicar las mejoras.
...
## También arreglar: death_date no se guarda

**Problema:** el campo `death_date` no llega a la BD porque el schema
de Joi en el backend lo rechaza cuando llega como string vacío.

**Archivo:** `apps/backend/src/middleware/validate.js`

Verificar que el schema `person` tiene exactamente esto:
...
person: Joi.object({
  ...
  death_date: Joi.date().optional().allow(null).allow(''),  // ← el allow('') es clave
  is_deceased: Joi.boolean().optional(),
  status: Joi.string().optional(),
  instagram: Joi.string().optional().allow('').allow(null),
  facebook: Joi.string().optional().allow('').allow(null),
  linkedin: Joi.string().optional().allow('').allow(null),
}),
```

Y en `PersonsPanel.jsx` en el `handleSubmit`, asegurarse de que
`death_date` se envía como `null` cuando está vacío:
...
```
1. Constellation.jsx  → reemplazar completo (arregla clicks, drag, filtro, exportar)
2. validate.js        → actualizar schema person (arregla death_date)
3. PersonsPanel.jsx   → actualizar handleSubmit (arregla death_date)
4. Redesplegar backend en Render (validate.js)
5. Redesplegar frontend en Netlify (Constellation.jsx)
```

## Sobre las fotos en la exportación

Las fotos de Cloudinary no aparecen en el PNG exportado por una
limitación de seguridad del navegador (CORS en canvas). No es un bug
del código — es una restricción del navegador que impide dibujar
imágenes externas en canvas y luego exportarlas.

La constelación se exporta correctamente con todos los círculos,
líneas, labels y colores. Solo faltan las fotos de perfil.
Para incluirlas haría falta un servidor que haga la conversión
(Puppeteer/headless Chrome). Por ahora la exportación sin fotos
